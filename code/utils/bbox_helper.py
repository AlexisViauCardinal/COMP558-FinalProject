...
def data2bboxes(data):
    ...
    for title, dataset in data.items():
        ...
        for class_dir in dataset['dirs']:
            with open(dataset['url']+'../annotations/'+class_dir+'.txt') as f:
...
def visualise_bbox(gt_bbox, bbox, file, class_dir, s, dataset):
    frame_color  = cv2.imread(dataset+class_dir+"/"+file)
    x1, y1, x2, y2 = get_bbox_points(bbox)
    x1, y1, x2, y2 = x1*s, y1*s, x2*s, y2*s

    gt_x1, gt_y1, gt_x2, gt_y2 = get_bbox_points(gt_bbox)
    gt_x1, gt_y1, gt_x2, gt_y2 = gt_x1*s, gt_y1*s, gt_x2*s, gt_y2*s

    # Draw the predicted bounding box (yellow)
    cv2.rectangle(frame_color, (x1, y1), (x2, y2), (0, 255, 255), 2)

    # Draw the ground truth bounding box (green)
    cv2.rectangle(frame_color, (gt_x1, gt_y1), (gt_x2, gt_y2), (0, 255, 0), 2)

    # Add labels to the bounding boxes
    cv2.putText(frame_color, "Predicted", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
    cv2.putText(frame_color, "Ground Truth", (gt_x1, gt_y1 - 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
